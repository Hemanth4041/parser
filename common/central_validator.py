...
class CentralValidator:
    ...
    def _validate_transaction_type(self, value: Any, field_name: str) -> Optional[str]:
        ...
        if str_value not in self.VALID_TRANSACTION_TYPES:
            return f"CRITICAL: Invalid transaction type in '{field_name}': '{value}'. Must be one of: CREDIT, DEBIT, C, D, CRDT, DBIT. File load rejected."
    ...
    def _validate_balance_integrity(self, row: Dict, all_rows: List[Dict] = None) -> Tuple[Optional[str], Optional[str]]:
        ...
        MANDATORY CHECK - FAIL if calculation doesn't match
        ...
        # Step 1: Check if both are missing - CRITICAL ERROR
        ...
        # Step 2: If only one is missing, issue warning but don't do calculation
        if opening_is_null:
        ...
        # Step 3: Both opening and closing exist - perform balance calculation validation
        if all_rows is not None:
            try:
                ...
                # Filter transactions for this org_id, account_number, and date
